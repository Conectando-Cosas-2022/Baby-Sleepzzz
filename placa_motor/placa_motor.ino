...
const char* token = "A13xQuigZHfMl930TkTU";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (_topic.startsWith("v1/devices/me/rpc/request/")) { // El servidor "me pide que haga algo" (RPC)
    ...
    if (metodo=="checkMotor") {  // Chequear el estado del dispositivo. Se debe responder utilizando el mismo request_number
      
      char outTopic[128];
      ("v1/devices/me/rpc/response/"+_request_id).toCharArray(outTopic,128);
      
      DynamicJsonDocument resp(256);
      resp["status"] = motor_state;
      char buffer[256];
      serializeJson(resp, buffer);
      client.publish(outTopic, buffer);
      
    } else if (metodo=="controlMotor") {  // Establecer el estado del led y reflejar en el atributo relacionado
      Serial.println("control Motor");
      bool motor_state = incoming_message["params"]; // Leer los parámetros del método
      if(motor_state){
        motor(100);  
      } else {
        motor(0);
    ...
    } else if(metodo == "controlMotorVariable"){
      int speed = incoming_message["params"];
      motor(speed);
